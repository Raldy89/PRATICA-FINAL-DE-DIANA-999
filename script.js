...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const quickContactStatus = document.getElementById('quick-contact-status');
    ...
    if (quickContactForm && quickContactStatus) {
        quickContactForm.addEventListener('submit', function(e) {
            ...
            const correo = document.getElementById('consulta-correo').value.trim();
            const asunto = document.getElementById('consulta-asunto').value.trim();
            const mensaje = document.getElementById('consulta-mensaje').value.trim();

            if (!nombre || !telefono || !correo || !asunto || !mensaje) {
                showNotification('Completa el formulario de contacto');
                return;
            }
            ...
            const consultasGuardadas = JSON.parse(localStorage.getItem('consultasContacto') || '[]');
            consultasGuardadas.push(consulta);
            localStorage.setItem('consultasContacto', JSON.stringify(consultasGuardadas));

            quickContactStatus.innerHTML = `
                <div class="contact-form-success">
                    <strong>Formulario enviado correctamente.</strong>
                    <p>Gracias, ${escapeHtml(nombre)}. Recibimos tu mensaje sobre ${escapeHtml(asunto)}.</p>
                    <p>Podemos responderte al ${escapeHtml(telefono)} o al correo ${escapeHtml(correo)}.</p>
                </div>
            `;

            quickContactForm.reset();
            quickContactStatus.scrollIntoView({
                behavior: 'smooth',
                block: 'nearest'
            });
            showNotification('Tu consulta fue enviada');
        });
    }
});
